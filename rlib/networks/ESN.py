...
from networks import dynamic_masked_rnn
...
def ESN(input, cell_size, num_inputs, fold_output=True, time_major=True):
    hidden_in = tf.placeholder(tf.float32, [None, cell_size], name='hidden_in')
    esn_cell = EchoStateRNNCell(cell_size, num_inputs)

    lstm_output, hidden_out = tf.compat.v1.nn.dynamic_rnn(esn_cell, input, initial_state=hidden_in, time_major=time_major)
    if fold_output:
        lstm_output = tf.reshape(lstm_output, shape=[-1, cell_size], name='folded_lstm_output')
    return lstm_output, hidden_in, hidden_out
