...
from rlib.utils.utils import one_hot, fold_batch, rolling_stats
from rlib.utils.SyncMultiEnvTrainer import SyncMultiEnvTrainer
from rlib.utils.VecEnv import*
from rlib.Curiosity.Curiosity import ICM

class ActorCritic_LSTM(object):
    def __init__(self, model_head, input_shape, action_size, num_envs, cell_size, intr_coeff=0.5, extr_coeff=1.0,
        ...
        with tf.variable_scope('losses'):
            ...
            tot_Adv = extr_coeff * Advantage_extr + intr_coeff * Advantage_intr
...
class RND(object):
    def __init__(self, policy_model, target_model, input_shape, action_size, num_envs, cell_size, policy_importance, reward_scale, intr_coeff=0.5, extr_coeff=1.0, lr=1e-3, lr_final=1e-3, decay_steps=6e5, grad_clip = 0.5, policy_args ={}, RND_args={}):
        ...
        with tf.variable_scope('ActorCritic', reuse=tf.AUTO_REUSE):
            self.train_policy = ActorCritic_LSTM(policy_model, input_shape, action_size, num_envs, cell_size, intr_coeff=intr_coeff, extr_coeff=extr_coeff, **policy_args)
        ...
        with tf.variable_scope('target_model'):
            target_state = target_model(self.next_state, trainable=False, **RND_args)
        
        with tf.variable_scope('predictor_model'):
            pred_next_state = target_model(self.next_state, **RND_args)
...
class RND_LSTM_Trainer(SyncMultiEnvTrainer):
...
def main(env_id):
    ...
    curiosity = RND_LSTM_Trainer(envs = envs,
                                  model = model,
...
if __name__ == "__main__":
    env_id_list = ['MontezumaRevengeDeterministic-v4', 'FreewayDeterministic-v4', 'VentureDeterministic-v4']
