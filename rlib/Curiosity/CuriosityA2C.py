...
from rlib.utils.wrappers import AtariEnv, DummyEnv
...
def main(env_id):
    ...
    if any(env_id in s for s in classic_list):
        ...
        val_envs = [
            AtariEnv(
                gym.make(env_id), k=4, rescale=84, episodic=False, reset=reset, clip_reward=False
            )
            for i in range(1)
        ]
        envs = BatchEnv(
