...
class ICM(object):
    def __init__(self, model_head, input_size, action_size, **model_head_args):    
        ...
        with tf.variable_scope('encoder_network'):
            self.phi1 = model_head((self.state - self.state_mean) / self.state_std,  **model_head_args)
        with tf.variable_scope('encoder_network', reuse=True):
            self.phi2 = model_head((self.next_state - self.state_mean) / self.state_std,  **model_head_args)
