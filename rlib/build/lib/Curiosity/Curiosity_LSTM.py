...
from utils.utils import one_hot, fold_batch
from SyncMultiEnvTrainer import SyncMultiEnvTrainer
from utils.VecEnv import*
from Curiosity.Curiosity import ICM
...
def main(env_id):
    ...
    if any(env_id in s for s in classic_list):
        ...
        val_envs = [AtariEnv(gym.make(env_id), k=4, rescale=42, episodic=False, reset=reset, clip_reward=False) for i in range(16)]
        envs = BatchEnv(AtariEnv, env_id, num_envs, rescale=42, blocking=False , k=4, reset=reset, episodic=False, clip_reward=True)
    ...
    model = Curiosity_LSTM(nature_cnn,
                      nature_cnn,
